...
class MVRecon(pl.LightningModule):
    def __init__(
        ...
        if lrm_path is not None:
            ...
            for k,v in lrm_ckpt["state_dict"].items():
                name = k.replace("lrm_generator.","") # 去掉前缀
                new_state_dict[name]=v
            print(len(new_state_dict.items()), len(lrm_ckpt["state_dict"].items()))
            self.lrm_generator.load_state_dict(new_state_dict, strict=False)

        # if lrm_path is not None:
        #     lrm_ckpt = torch.load(lrm_path)
        #     self.lrm_generator.load_state_dict(lrm_ckpt["state_dict"], strict=False)
    ...
    def prepare_batch_data(self, batch):
        ...
        # masks
        lrm_generator_input['loss_mask'] = batch['mask'] # (batch_size, 6+4)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, loss_dict = self.compute_loss(render_out, render_gt, lrm_generator_input['loss_mask'])

        self.log_dict(loss_dict, prog_bar=True, logger=True, on_step=True, on_epoch=True)

        if self.global_step % 1000 == 0 and self.global_rank == 0:     #1000
    ...
    def compute_loss(self, render_out, render_gt, loss_mask):
        ...
        loss_mask = rearrange(loss_mask, 'b n ... -> (b n) ...')   # loss_mask (b 10) -> (b*10)
        #print("1111111111111",render_images.shape, loss_mask.shape)   #  torch.Size([10, 3, 192, 192]) torch.Size([10])

        # mask the views not need to compute loss
        render_images, target_images = render_images[loss_mask.bool()], target_images[loss_mask.bool()]
