...
class ObjaverseData(Dataset):
    ...
    def load_xyz_occ_sdf(self, path, surf_path):
        ...
        surf_occ = torch.full((surf_xyz.size(0),), 0.5)  # Surface的occupancy设置为0.5
        ...
        occupancy = torch.cat([occupancy, surf_occ], dim=0)
        sdf = torch.cat([sdf, surf_sdf], dim=0)
        grad = torch.cat([grad, surf_grad], dim=0)

        
        # 创建 mask：假设总共有 300w 数据
        total_points = xyz.size(0)  # 计算总点数
        mask = torch.zeros(total_points, dtype=torch.int32)  # 初始化为0的 mask

        # 前 100w 为 2
        mask[:1000000] = 2

        # 中间 100w 为 1
        mask[1000000:2000000] = 1

        # 最后 100w 为 0
        mask[2000000:] = 0
        return xyz, occupancy, sdf, grad, mask
    ...
    def __getitem__(self, index):
        ...
        xyz, occupancy, sdf, grad, mask = self.load_xyz_occ_sdf(occ_path, surf_path)
...
class ValidationData(Dataset):
    ...
    def load_xyz_occ_sdf(self, path, surf_path):
        ...
        # 中间 100w 为 2
        mask[1000000:2000000] = 2  # volume

        # 最后 100w 为 0
        mask[2000000:] = 0 # surface
        return xyz, occupancy, sdf, grad, mask
