...
class MVRecon(pl.LightningModule):
            ...
            if os.path.basename(lrm_path) == 'instant_nerf_large.ckpt':  # 复用原始lrm权重
                ...
                for k, v in sd.items():
                    if k.startswith('lrm_generator.synthesizer.decoder.net_sdf.'):    # last layer
                        ...
                        # we reverse the sign of density filed to initialize SDF field.  
                        # -(w*x + b - t) = (-w)*x + (t - b)
                        if k.startswith('lrm_generator.synthesizer.decoder.net_sdf.6.'):
                            if 'weight' in k:
                                sd_fc[k.replace('net_sdf.', 'net.')] = torch.cat([v, -v], dim=0) 
                            else:
                                sd_fc[k.replace('net_sdf.', 'net.')] = torch.cat([v, 10.0-v], dim=0) 
                        else:
    ...
    def random_crop(self, query_xyz, sdf_gt, occ_gt, grad_gt, mask, min_points=400000):
        query_xyz_list = []
        sdf_gt_list = []
        occ_gt_list = []
        grad_gt_list = []
        ...
        for i in range(query_xyz.size(0)):  # 遍历每个 batch 中的样本
            ...
            mask_sample_0 = mask[i].to(self.device)
            ...
            occ_gt_sample = occ_gt_sample_0[mask_]
        ...
        return query_xyz, sdf_gt, occ_gt, grad_gt, mask
    ...
    def prepare_batch_data(self, batch):
        ...
        query_xyz, sdf_gt, occ_gt, grad_gt, mask = self.random_crop(query_xyz, sdf_gt, occ_gt, grad_gt, mask)

        return lrm_generator_input, query_xyz, sdf_gt, occ_gt, grad_gt, mask
    ...
    def prepare_validation_batch_data(self, batch):
        ...
        return lrm_generator_input, query_xyz, sdf_gt, occ_gt, grad_gt
    ...
    def diff_gradient(self, lrm_generator_input, transformed_xyz, epsilon=0.01):
        """
        ...
        for i, axis in enumerate([0, 1, 2]):  # 分别处理 x, y, z 轴
            ...
            gradients[..., i] = ((sdf_pos - sdf_neg) / (2 * epsilon)).squeeze(-1)
    ...
    def training_step(self, batch, batch_idx):
        lrm_generator_input, query_xyz, sdf_gt, occ_gt, grad_gt, mask = self.prepare_batch_data(batch)   # torch.Size([b, 200000, 3]), # torch.Size([b, 200000]) 
        ...
        loss, loss_dict = self.compute_loss(sdf_out, occ_out, sdf_gt.unsqueeze(-1), occ_gt.unsqueeze(-1), \
                gradients, grad_gt, mask, 'train')
        self.log_dict(loss_dict, prog_bar=True, logger=True, on_step=True, on_epoch=False)
        
        if self.global_step % self.train_check_interval == 0:     #1000

            print("train_sdf min: {:.1f}, max: {:.1f},  GT min: {:.1f}, max: {:.1f}".format(
                sdf_out.min().item(), sdf_out.max().item(),
                sdf_gt.unsqueeze(-1).min().item(), sdf_gt.unsqueeze(-1).max().item()
            ))

            print("train_occ min: {:.1f}, max: {:.1f},  GT min: {:.1f}, max: {:.1f}".format(
                occ_out.min().item(), occ_out.max().item(),
                occ_gt.unsqueeze(-1).min().item(), occ_gt.unsqueeze(-1).max().item()
            ))
    ...
    def compute_loss(self, sdf_out, occ_out, sdf_gt, occ_gt,\
                    gradients, grad_gt, mask, prefix):
        ...
        # surface loss
        grad_loss_surf = (gradients[mask_surface] - grad_gt[mask_surface]).norm(2, dim=-1).mean()
        sdf_loss_surf = (sdf_out[mask_surface]).abs().mean()
        # print(gradients[mask_surface][:2])
        # print(grad_gt[mask_surface][:2])

        # volume loss
        criteria = torch.nn.BCEWithLogitsLoss()
        occ_loss = criteria(occ_out[~mask_surface], occ_gt[~mask_surface]).mean()
        eikonal_loss = ((gradients.norm(2, dim=-1) - 1) ** 2).mean()
        sdf_loss_vol = (sdf_out[~mask_surface] - sdf_gt[~mask_surface]).abs().mean()

        loss_dict.update({f'{prefix}_surf_sdf': sdf_loss_surf, f'{prefix}_surf_grad': grad_loss_surf, \
                        f'{prefix}_occ_loss': occ_loss, f'{prefix}_sdf_vol': sdf_loss_vol, f'{prefix}_eik': eikonal_loss, \
                        })


        loss =  occ_loss \
                + 0.5 * sdf_loss_surf + 0.1 * sdf_loss_vol \
                #+ 0.1 * grad_loss_surf \
                #+ 0.1 * eikonal_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        lrm_generator_input, query_xyz, sdf_gt, occ_gt, grad_gt = self.prepare_validation_batch_data(batch)
        ...
        sdf_out, occ_out = self.forward(lrm_generator_input, transformed_xyz) # torch.Size([b, 200000, 1])
        print("val_sdf min: {:.1f}, max: {:.1f}".format(sdf_out.min().item(), sdf_out.max().item()))
        print("val_occ min: {:.1f}, max: {:.1f}".format(occ_out.min().item(), occ_out.max().item()))
