...
class SDFDecoder(nn.Module):
    ...
    def forward(self, sampled_features):
        ...
        x = self.net(x)
        x = x.view(N, M, -1)
        # sdf_prob = torch.tanh(x)  # Uses sigmoid clamping from MipNeRF
        sdf_prob = torch.sigmoid(x)
        return {'sdf': sdf_prob}
...
class TriplaneSynthesizer(nn.Module):
    ...
    def __init__(self, triplane_dim: int, samples_per_ray: int):
        ...
        self.decoder = SDFDecoder(n_features=triplane_dim)
