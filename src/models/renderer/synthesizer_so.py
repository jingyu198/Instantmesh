...
class SODecoder(nn.Module):
    ...
    def forward(self, sampled_features):
        ...
        x = self.net(x)
        x = x.view(N, M, -1)
        sdf_prob = torch.tanh(x[..., 0:1]) # Uses sigmoid clamping from MipNeRF
        occ_prob = torch.sigmoid(x[..., 1:2])  # Uses sigmoid clamping from MipNeRF
        #occ_prob = x[..., 1:2]
        return {'sdf': sdf_prob, 'occupancy': occ_prob}
...
class TriplaneSynthesizer(nn.Module):
    ...
    def __init__(self, triplane_dim: int, samples_per_ray: int):
        ...
        self.decoder = SODecoder(n_features=triplane_dim)
    ...
    def forward(self, planes, points: torch.Tensor, chunk_size: int = 2**20):
        ...
        return point_features['sdf'], point_features['occupancy']
