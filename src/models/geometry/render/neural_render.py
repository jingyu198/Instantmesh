...
#                temp_channle = -channel[mask]

                temp_channle = channel[mask] - channel[mask].min()
                channel_max = torch.max(temp_channle)
                channel[mask] = temp_channle / channel_max
                channel[mask] = 1- channel[mask]
            normalized_depth[i] = channel

        depth = normalized_depth.unsqueeze(-1)


        
#        print(depth.max(),depth.min(),"before")
#        depth = depth - depth.min()
#        depth_max = torch.max(depth)
#        depth = depth/ depth_max
#        print(depth.max , torch.max(depth))
#        print(depth.max(),depth.min(),"after")
#        depth = torch.lerp(torch.zeros_like(depth), depth / depth.max(), hard_mask.float())
#        depth = 1 - depth 
