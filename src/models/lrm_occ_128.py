...
from .decoder.transformer_128 import TriplaneTransformer
from .renderer.synthesizer_occ_128 import TriplaneSynthesizer
...
class InstantNeRF(nn.Module):
    ...
    def forward_planes(self, images, cameras):
        ...
        #print("triplane_size: ", planes.shape)
        return planes
    ...
    def extract_mesh(
        ...
        **kwargs,
        ...
        x = torch.linspace(-1, 1, mesh_resolution)
        y = torch.linspace(-1, 1, mesh_resolution)
        z = torch.linspace(-1, 1, mesh_resolution)
        ...
        grid_out = self.synthesizer.forward(
            planes = planes,
            points = grid,
        ).view(mesh_resolution, mesh_resolution,mesh_resolution)

        grid_out = grid_out.cpu().detach().numpy()
        grid_out = (grid_out > 0.5).astype(np.uint8)
        vertices, faces = mcubes.marching_cubes(grid_out, 0.5)
